...
Name | Type | Description
--------- | --------- | ---------
relation | Integer | 서비스와 사용자의 관계에 대한 노출조건을를 나타냄.<br>0: 모든 대상에게 노출<br>1: 유저가 서비스와 관계가 없을 경우에만 노출 (미가입자 혹은 친구 아님, 서비스->사용자 단방향 친구 개념)<br>2: 유저가 서비스와 관계가 있을 경우에만 노출 (가입자 혹은 친구 관계, 서비스<->사용자 쌍방 친구 개념) 
